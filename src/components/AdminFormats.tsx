...
export default function AdminFormats() {
  ...
  const [requiresPhysicalShipping, setRequiresPhysicalShipping] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const data = { name, requiresPhysicalShipping };
      await createPhysicalFormat(data);
      setName(''); setRequiresPhysicalShipping(false); loadData(); alert("Catálogo de formatos actualizado.");
    } catch (err: any) { alert("Error: " + err.message); }
  };

  const handleDelete = async (id: number) => {
    if(!window.confirm("¿Seguro que deseas eliminar este formato físico?")) return;
    try { await deletePhysicalFormat(id, 'Admin'); loadData(); } catch (err: any) { alert("Error: " + err.message); }
  };
      ...
      <form onSubmit={handleSubmit} className={`bg-white/[0.04] border border-white/10 rounded-xl p-6 mb-8 max-w-2xl`}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 bg-white/[0.02] border border-white/[0.06] rounded-lg p-4">
            <input type="checkbox" id="shippingOpt" checked={requiresPhysicalShipping} onChange={e => setRequiresPhysicalShipping(e.target.checked)} className="w-5 h-5 rounded border-2 border-white/20 bg-white/[0.04] checked:bg-violet-500 checked:border-violet-500 cursor-pointer accent-violet-500" />
            <label htmlFor="shippingOpt" className="text-sm font-medium text-slate-300 cursor-pointer select-none">¿Requiere envío físico? (Ej. Vinilos, CDs)</label>
          </div>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-white/[0.08]">
              ...
              <th className="p-4 text-xs font-semibold uppercase text-slate-400 tracking-wider text-center">Envío Físico</th>
            ...
            {formats.map((f, idx) => (
              <tr key={f.id} className={`border-b border-white/[0.06] hover:bg-white/[0.04] transition-colors ${idx % 2 !== 0 ? 'bg-white/[0.02]' : ''}`}>
                ...
                <td className="p-4 text-center">
                  <span className={`px-2 py-1 rounded text-[10px] font-bold border uppercase tracking-wider ${f.requiresPhysicalShipping ? 'bg-violet-500/15 text-violet-400 border-violet-500/20' : 'bg-slate-500/15 text-slate-400 border-slate-500/20'}`}>{f.requiresPhysicalShipping ? 'SÍ' : 'NO'}</span>
                </td>
